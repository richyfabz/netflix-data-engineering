...
ALTER TABLE analytics.dim_country
...
ALTER TABLE analytics.bridge_title_country
...
CREATE SEQUENCE IF NOT EXISTS analytics.dim_title_sk_seq;

SELECT setval(
    'analytics.dim_title_sk_seq',
    COALESCE(
        (SELECT MAX(title_sk) FROM analytics.dim_title),
        0
    ) + 1,
    false
);
...
SET DEFAULT nextval('analytics.dim_title_sk_seq');

ALTER SEQUENCE analytics.dim_title_sk_seq
OWNED BY analytics.dim_title.title_sk;
...
-- Person dimension.
CREATE SEQUENCE IF NOT EXISTS analytics.dim_person_sk_seq;

SELECT setval(
    'analytics.dim_person_sk_seq',
    COALESCE(
        (SELECT MAX(person_sk) FROM analytics.dim_person),
        0
    ) + 1,
    false
);
...
SET DEFAULT nextval('analytics.dim_person_sk_seq');

ALTER SEQUENCE analytics.dim_person_sk_seq
OWNED BY analytics.dim_person.person_sk;


-- Genre dimension.
CREATE SEQUENCE IF NOT EXISTS analytics.dim_genre_sk_seq;

SELECT setval(
    'analytics.dim_genre_sk_seq',
    COALESCE(
        (SELECT MAX(genre_sk) FROM analytics.dim_genre),
        0
    ) + 1,
    false
);
...
SET DEFAULT nextval('analytics.dim_genre_sk_seq');

ALTER SEQUENCE analytics.dim_genre_sk_seq
OWNED BY analytics.dim_genre.genre_sk;
